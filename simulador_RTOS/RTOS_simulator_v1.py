"""
rocket_visualizer.py  —  v11.2
Adicionado desenhar_temp_overlay: exibe a temperatura interna do MPU6050
no canto inferior esquerdo quando o sensor está conectado.
"""
...
class MaquinaMissao:
    ...
    ESTADO_IDLE     = "IDLE"
    ESTADO_INTRO    = "INTRO"      # painel de boas-vindas
    ESTADO_AVISO    = "AVISO"
    ESTADO_CHUVA    = "CHUVA"
...
# ============================================================
# OVERLAY DE INTRODUÇÃO À MISSÃO
# ============================================================
...
# ============================================================
# TEMPERATURA OVERLAY — inferior esquerdo
# ============================================================

def desenhar_temp_overlay(surface, leitor, margem=14):
    """Exibe a temperatura interna do MPU6050 no canto inferior esquerdo.

    Só renderiza quando o sensor está conectado. Cor indica faixa operacional:
      verde  (< 45 °C) — normal
      amarelo (45–60 °C) — atenção
      vermelho (> 60 °C) — temperatura alta
    """
    if not leitor.conectado:
        return

    temp  = leitor.temperatura
    W, H  = surface.get_size()
    t_now = time.time()

    # ── Cores por faixa ───────────────────────────────────────
    if temp < 45.0:
        cor_val = (80,  220, 160, 240)   # verde
        cor_led = (80,  220, 160)
    elif temp < 60.0:
        cor_val = (255, 200,  40, 240)   # amarelo
        cor_led = (255, 200,  40)
    else:
        cor_val = (255,  80,  60, 240)   # vermelho
        cor_led = (255,  80,  60)

    f_tag  = fontes().label(11, bold=True)
    f_val  = fontes().dados(22)
    f_unit = fontes().label(14)

    tag_s  = f_tag.render("TEMP SENSOR", True, (80, 140, 200))
    val_s  = f_val.render(f"{temp:.1f}", True, cor_val[:3])
    unit_s = f_unit.render("°C", True, cor_val[:3])

    # ── Dimensões do box ──────────────────────────────────────
    pad   = 10
    w_val = val_s.get_width() + 5 + unit_s.get_width()
    w_box = max(tag_s.get_width() + pad + 18, w_val) + pad * 2
    h_box = pad + tag_s.get_height() + 6 + max(val_s.get_height(), unit_s.get_height()) + pad

    bx = margem
    by = H - margem - h_box

    # ── Background ────────────────────────────────────────────
    bg = pygame.Surface((w_box, h_box), pygame.SRCALPHA)
    pygame.draw.rect(bg, (8, 12, 22, 188),        (0, 0, w_box, h_box), border_radius=8)
    pygame.draw.rect(bg, (*cor_led, 95),           (0, 0, w_box, h_box), 1, border_radius=8)

    # ── LED pulsante ──────────────────────────────────────────
    led_a = int(150 + 90 * math.sin(t_now * 2.5))
    pygame.draw.circle(bg, (*cor_led, led_a), (pad + 6, pad + tag_s.get_height() // 2 + 1), 4)
    pygame.draw.circle(bg, (255, 255, 255, 55), (pad + 5, pad + tag_s.get_height() // 2), 2)

    # ── Tag ───────────────────────────────────────────────────
    tag_s.set_alpha(200)
    bg.blit(tag_s, (pad + 18, pad))

    # ── Valor + unidade alinhados na base ─────────────────────
    vy      = pad + tag_s.get_height() + 6
    val_a   = cor_val[3]
    val_s.set_alpha(val_a)
    unit_s.set_alpha(val_a)
    bg.blit(val_s,  (pad, vy))
    bg.blit(unit_s, (pad + val_s.get_width() + 4,
                     vy + val_s.get_height() - unit_s.get_height() + 2))
...
def main():
    ...
    pygame.display.set_caption("Rtos Simulator v11.2")
    ...
    while True:
        ...
        desenhar_temp_overlay(overlay_surf, leitor, margem=14)   # ← temperatura inferior esquerdo
